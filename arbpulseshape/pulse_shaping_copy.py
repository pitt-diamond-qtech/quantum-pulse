...
if __name__ == "__main__":
    ...
    n = 5
    phi,alpha = alphafromphi(np.pi/2 - 0.0001,n)#50)
    max_val = 0
    tt_write = np.zeros([n, 500])

    kappa_write = np.zeros([n, 500])
    ...
    for i in range(0,len(alpha)):
       ...
       # print alpha[i]
        ...
        ll_max = np.pi  # Maximum value of the parametrization (can be pi or 2 pi)
        ...
        plt.show()
    kappa_write = kappa_write/max_val

    for i in range(0,n):
        file_name = 'arb' + str(i) + '.txt'
        with open(os.path.join(dir_path, file_name), 'w') as outfile:
            for ind, t in enumerate(tt_write[i]):
                outfile.write("%s, %s, %s \n" % (ind, tt_write[i][ind], kappa_write[i][ind]))
